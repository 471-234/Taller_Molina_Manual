...
{
    public partial class ServicioMecanica : UserControl
    {
        public ServicioMecanica()
        {
            InitializeComponent();
            CargarServicios();
        }
        ...
        {
            dgvMecanica.DataSource = DBHelper.Query(
                "SELECT ServicioID, Nombre, Descripcion, Precio FROM Servicios WHERE Categoria='Mecanica'");
        }
        ...
        {
            if (dgvMecanica.SelectedRows.Count == 0) return;

            int id = Convert.ToInt32(dgvMecanica.SelectedRows[0].Cells["ServicioID"].Value);
            ...
            DBHelper.ExecNonQuery(
                @"UPDATE Servicios SET Nombre=@n,Descripcion=@d,Precio=@p
                  WHERE ServicioID=@id AND Categoria='Mecanica'",
                DBHelper.P("@n", nombre),
                DBHelper.P("@d", desc),
                DBHelper.P("@p", precio),
                DBHelper.P("@id", id));
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            dgvMecanica.DataSource = DBHelper.Query(
                @"SELECT ServicioID,Nombre,Descripcion,Precio 
                  FROM Servicios 
                  WHERE Categoria='Mecanica' AND Nombre LIKE CONCAT('%',@t,'%')",
